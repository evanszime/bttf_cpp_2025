    ...
    BeginTextureMode(_renderTarget);
        ...
        for (auto& d : drawables) {
            ...
            case Drawable::SPRITE: {
                ...
                if (s && (int)_global->_allSprites.size() > s->texturePath) {
                    DrawTexturePro(
                        ...
                        { px + s->offsetX, py + s->offsetY, s->width * s->scale, s->height * s->scale },
                        { (s->width * s->scale) / 2.f, (s->height * s->scale) / 2.f },
                        s->rotation,
                        s->tint
                    );
                }
            ...
            case Drawable::TEXT: {
                ...
                if (t->field) {
                    Color bg = t->isTaping ? RED : BLACK;
                    DrawRectangle((int)px-5, (int)py-5,
                        10 + MeasureText("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", (int)t->size),
                        (int)t->size+10, bg);
                }
